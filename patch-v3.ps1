        ...
        const onMessage = (e) => {
          if (e.data && e.data.type === 'ctp-engine-view') {
            ...
            if (!code) {
              ...
              // Map S/01 -> stages[0], S/02 -> stages[1], etc.
...
$hero = Get-Content "assets/animation/hero-scenes.jsx" -Raw
$newHtml = $html.Replace('___HERO_SCENES___', $hero)
Set-Content "assets/animation/iframe-engine-v3.html" $newHtml -Encoding UTF8
