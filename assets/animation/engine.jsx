...
const TurbineDisc = ({ x = 0, r = 40, ry = 14, fins = 24 }) => {
  ...
  return (
    <g>
      ...
      <ellipse className="part-stroke" cx={x} cy={0} rx={ry * 0.25} ry={r * 0.25} fill="rgba(47,212,122,.15)"/>
    </g>
  );
};
    ...
    >
      <defs>
        <pattern id="bp-grid" width="40" height="40" patternUnits="userSpaceOnUse">
          <path d="M 40 0 L 0 0 0 40" fill="none" stroke="rgba(47,212,122,.06)" strokeWidth=".6"/>
        </pattern>
        <pattern id="bp-grid-fine" width="8" height="8" patternUnits="userSpaceOnUse">
          <path d="M 8 0 L 0 0 0 8" fill="none" stroke="rgba(47,212,122,.025)" strokeWidth=".4"/>
        </pattern>
        <radialGradient id="cglow" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="rgba(47,212,122,.06)"/>
          <stop offset="100%" stopColor="rgba(47,212,122,0)"/>
        </radialGradient>
      </defs>

      <rect width="1200" height="720" fill="url(#bp-grid-fine)"/>
      <rect width="1200" height="720" fill="url(#bp-grid)"/>
      <ellipse cx={cx} cy={cy} rx="500" ry="260" fill="url(#cglow)"/>

      {/* Reference axis - subtle horizontal centerline */}
      <line x1="80" y1={cy} x2="1120" y2={cy}
            stroke="rgba(47,212,122,.15)" strokeWidth=".8" strokeDasharray="6 6"/>

      {/* Outer frame */}
      <rect x="60" y="60" width="1080" height="600" fill="none"
            stroke="rgba(47,212,122,.12)" strokeWidth="1" strokeDasharray="4 4"/>

      {/* Corner ticks */}
      {[[60,60,1,1],[1140,60,-1,1],[60,660,1,-1],[1140,660,-1,-1]].map(([x,y,sx,sy],i)=>(
        <g key={i}>
          <line x1={x} y1={y} x2={x+18*sx} y2={y} stroke="rgba(47,212,122,.5)" strokeWidth="1.5"/>
          <line x1={x} y1={y} x2={x} y2={y+18*sy} stroke="rgba(47,212,122,.5)" strokeWidth="1.5"/>
        </g>
      ))}
